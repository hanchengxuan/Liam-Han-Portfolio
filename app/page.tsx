...
export default function Home() {
  return <main className="mx-auto min-h-screen max-w-6xl px-4 py-6 md:px-8 md:py-10">
    ...
    <header id="top" className="mb-12 grid gap-8 md:grid-cols-[1.25fr_0.75fr] md:items-center"><div><p className="mb-4 text-terminal">$ whoami</p><h1 className="text-4xl font-black leading-tight text-white md:text-6xl">Liam <span className="text-terminal">(Chengxuan)</span> Han</h1><p className="mt-6 max-w-2xl text-lg leading-8 text-emerald-100/80">Full stack software engineer building AI-enabled web products, conversational workflows, and automation systems that survive real production traffic.</p><div className="mt-8 flex flex-wrap gap-3"><a href="#projects" className="border border-terminal bg-terminal px-5 py-3 text-sm font-bold text-black hover:bg-white">View Projects</a><a href={resumeHref} download className="border border-terminal/50 px-5 py-3 text-sm font-bold text-terminal hover:border-white hover:text-white">Download Resume</a></div></div><div className="border border-terminal/25 bg-black/50 p-5 text-sm leading-7 text-terminal/85 shadow-glow"><p className="text-white">system.status</p><p>{'>'} role: full_stack_engineer</p><p>{'>'} focus: ai + web + automation</p><p>{'>'} stack: react + typescript + python</p><p>{'>'} infra: azure + docker + ci/cd</p><p className="mt-4 animate-pulse text-terminal">connection established_</p></div></header>
